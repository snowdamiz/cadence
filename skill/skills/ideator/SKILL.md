...
4. Run `cd "$PROJECT_ROOT" && python3 "$CADENCE_SCRIPTS_DIR/assert-workflow-route.py" --skill-name ideator --project-root "$PROJECT_ROOT"` and parse the JSON response.
5. If route assertion fails, stop and surface the exact error to the user.
6. Start from the user's input and briefly restate your understanding.
7. Accept two input modes:
   - Conversational discovery from a rough idea.
   - Project brief or project document provided up front.
8. If the user provides a project brief or document:
...
9. Ask exactly one question at a time. Never ask a batch of questions in a single turn.
10. After each user answer:
   - Summarize what changed in one short sentence.
   - Decide the next highest-leverage unknown.
   - Ask one natural follow-up question.
11. Keep discovery domain-agnostic and adaptive:
...
12. Build understanding until the idea is execution-ready. Cover the relevant dimensions for the domain, including:
...
20. Mention that research execution runs in a separate `researcher` phase with one pass per conversation and handoff between passes (`Start a new chat and say "continue research".`).
...
23. If the user requests revisions later, regenerate the payload, rerun `prepare-ideation-research.py`, and rerun `inject-ideation.py` from `PROJECT_ROOT`.
